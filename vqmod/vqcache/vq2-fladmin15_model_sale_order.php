...
'invoice_no_clean'              => $order_query->row['invoice_no'],
            	...
            	'invoice_filename' 		   => $order_query->row['invoice_filename'], 
...
LEFT JOIN " . DB_PREFIX . "order_status oos ON (o.order_status_id = oos.order_status_id) WHERE cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getOrders($data = array()) {
		$sql = "SELECT o.order_id, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, o.invoice_prefix, o.invoice_no, o.total,  o.currency_code, o.currency_value, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order` o";
...
if (!empty($data['filter_invoice_prefix'])) {
	...
	public function addOrderHistory($order_id, $data) {
		...
		if ($this->config->get('config_complete_status_id') == $data['order_status_id']) {
      	...
      	if ($data['notify']) {
			...
			$this->db->query("UPDATE " . DB_PREFIX . "order_history SET comment = '" . $this->db->escape($template->data['new_comment']) . "' WHERE order_history_id = '" . (int)$order_history_id . "'"); 
		}
	}
	...
	public function getEmailsByProductsOrdered($products, $start, $end) {
		...
		$query = $this->db->query("SELECT DISTINCT email, o.order_id, o.customer_id, o.store_id, o.language_id FROM `" . DB_PREFIX . "order` o LEFT JOIN " . DB_PREFIX . "order_product op ON (o.order_id = op.order_id) WHERE (" . implode(" OR ", $implode) . ") AND o.order_status_id <> '0'");
	...
	public function getTotalEmailsByProductsOrdered($products) {
		...
		/*$query = $this->db->query("SELECT DISTINCT email, o.order_id, o.customer_id, o.store_id, o.language_id FROM `" . DB_PREFIX . "order` o LEFT JOIN " . DB_PREFIX . "order_product op ON (o.order_id = op.order_id) WHERE (" . implode(" OR ", $implode) . ") AND o.order_status_id <> '0' LIMIT " . $start . "," . $end);	*/
